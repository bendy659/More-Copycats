...
object FencePartOverlay {
    ...
    private fun render(client: Minecraft) {
        ...
        val block = state.block as? CopycatFenceBlock ?: return
        ...
        val visibleSlots = CopycatFenceWallBlockEntity.FenceSlot.entries
            .filter { slot ->
                when (mode) {
        ...
        val zones = block.slotBoxes(state, pos)
            ...
            .mapIndexed { index, box ->
                Zone(
                    key = "more_copycats:fence_${mode.name.lowercase()}:${pos.asLong()}:${box.slot.name.lowercase()}:$index",
                    slot = box.slot,
                    aabb = box.aabb
                )
            }
    ...
    private fun isHoldingMaterialItem(stack: ItemStack): Boolean {
        ...
        if (block is CopycatFenceBlock) return false
