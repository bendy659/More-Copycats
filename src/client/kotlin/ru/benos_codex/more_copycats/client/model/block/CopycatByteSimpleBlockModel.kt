...
class CopycatByteSimpleBlockModel(state: BlockState, unbaked: BlockStateModel.UnbakedRoot) : CopycatModel(state, unbaked) {

    private fun renderFace(
        material: BlockState,
        be: CopycatByteBlockEntity,
        index: Int,
    ...
    override fun addPartsWithInfo(
        ...
        val be = p0.getBlockEntity(p1) as? CopycatByteBlockEntity ?: return
    ...
    private fun hasNeighbor(be: CopycatByteBlockEntity, index: Int, face: Direction): Boolean {
        val x = index and 1
        ...
        val ny = y + face.stepY
        val nz = z + face.stepZ
        if (nx !in 0..1 || ny !in 0..1 || nz !in 0..1) return false
        val nIndex = nx + (ny * 2) + (nz * 4)
    ...
    private fun getRegionBounds(be: CopycatByteBlockEntity, index: Int, face: Direction): RegionBounds {
