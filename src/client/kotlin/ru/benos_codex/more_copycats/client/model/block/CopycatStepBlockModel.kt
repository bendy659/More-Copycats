...
class CopycatStepBlockModel(state: BlockState, unbaked: BlockStateModel.UnbakedRoot) : CopycatModel(state, unbaked) {
    override fun addPartsWithInfo(
        ...
        val axis = state.getValue(CopycatStepBlock.AXIS)

        for (templatePart in templateParts) {
            val builder = QuadCollection.Builder()
            addPartRemapped(templatePart, builder, refsBySlot, materialsBySlot, axis, state, world, pos)
    ...
    private fun shouldSkipInternalFace(
        ...
        val adjacent = adjacentSlot(state, slot, direction) ?: return false
    ...
    private fun hasSlot(state: BlockState, slot: CopycatVerticalStepBlockEntity.Slot): Boolean = when (slot) {
        CopycatVerticalStepBlockEntity.Slot.P0 -> state.getValue(CopycatStepBlock.P0)
        CopycatVerticalStepBlockEntity.Slot.P1 -> state.getValue(CopycatStepBlock.P1)
        CopycatVerticalStepBlockEntity.Slot.P2 -> state.getValue(CopycatStepBlock.P2)
        CopycatVerticalStepBlockEntity.Slot.P3 -> state.getValue(CopycatStepBlock.P3)
    }
    ...
    private fun adjacentSlot(
        ...
        return when (state.getValue(CopycatStepBlock.AXIS)) {
            Direction.Axis.X -> when (slot) {
                CopycatVerticalStepBlockEntity.Slot.P0 -> when (direction) {
                    Direction.SOUTH -> CopycatVerticalStepBlockEntity.Slot.P1
                    Direction.UP -> CopycatVerticalStepBlockEntity.Slot.P2
                    else -> null
                }
                CopycatVerticalStepBlockEntity.Slot.P1 -> when (direction) {
                    Direction.NORTH -> CopycatVerticalStepBlockEntity.Slot.P0
                    Direction.UP -> CopycatVerticalStepBlockEntity.Slot.P3
                    else -> null
                }
                CopycatVerticalStepBlockEntity.Slot.P2 -> when (direction) {
                    Direction.SOUTH -> CopycatVerticalStepBlockEntity.Slot.P3
                    Direction.DOWN -> CopycatVerticalStepBlockEntity.Slot.P0
                    else -> null
                }
                CopycatVerticalStepBlockEntity.Slot.P3 -> when (direction) {
                    Direction.NORTH -> CopycatVerticalStepBlockEntity.Slot.P2
                    Direction.DOWN -> CopycatVerticalStepBlockEntity.Slot.P1
                    else -> null
                }
            }
            Direction.Axis.Z -> when (slot) {
                CopycatVerticalStepBlockEntity.Slot.P0 -> when (direction) {
    ...
    private fun isInnerSeamFace(quad: BakedQuad, direction: Direction): Boolean {
        ...
        return when (direction) {
            ...
            Direction.UP, Direction.DOWN -> isNear((bounds.minY + bounds.maxY) * 0.5f, 0.5f)
        }
    }
    ...
    private fun pickSlot(
        ...
        val cy = (quad.position0().y() + quad.position1().y() + quad.position2().y() + quad.position3().y()) * 0.25f
        val cz = (quad.position0().z() + quad.position1().z() + quad.position2().z() + quad.position3().z()) * 0.25f
        val direction = quad.direction()
        val top = splitAtHalf(cy, direction, negativeOnSeamFace = Direction.UP, positiveOnSeamFace = Direction.DOWN)

        return when (axis) {
            Direction.Axis.X -> {
                val south = splitAtHalf(cz, direction, negativeOnSeamFace = Direction.SOUTH, positiveOnSeamFace = Direction.NORTH)
                if (top) {
                    if (south) CopycatVerticalStepBlockEntity.Slot.P3 else CopycatVerticalStepBlockEntity.Slot.P2
                } else {
                    if (south) CopycatVerticalStepBlockEntity.Slot.P1 else CopycatVerticalStepBlockEntity.Slot.P0
                }
            }
            Direction.Axis.Z -> {
                val east = splitAtHalf(cx, direction, negativeOnSeamFace = Direction.EAST, positiveOnSeamFace = Direction.WEST)
                if (top) {
                    if (east) CopycatVerticalStepBlockEntity.Slot.P3 else CopycatVerticalStepBlockEntity.Slot.P2
                } else {
                    if (east) CopycatVerticalStepBlockEntity.Slot.P1 else CopycatVerticalStepBlockEntity.Slot.P0
                }
            }
            Direction.Axis.Y -> {
                val east = splitAtHalf(cx, direction, negativeOnSeamFace = Direction.EAST, positiveOnSeamFace = Direction.WEST)
